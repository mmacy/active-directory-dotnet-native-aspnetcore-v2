...
In this scenario, we protect a web API using the Microsoft identity platform. This ensures that the web API is only accessible to authenticated users. In these samples we work with apps that authenticate users using both **Work and school accounts** or personal **Microsoft accounts (MSA)**.
al
